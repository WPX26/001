...
13. [相册管理](#13-相册管理)
14. [通用接口](#14-通用接口)
15. [雷达（实时位置共享）](#15-雷达实时位置共享2026-09-20-一期)
...
GET /inspire/coords
```
**功能**：获取公开的生活池坐标点（红色标记，含自己上传的；王总 2026-09-03 拍板：自己上传的生活坐标也在灵感模式显示，工作池坐标归探索模式），按"已关注作者优先 → 热度 → 时间"排序。
...
### 5.3 购买坐标置顶（五席循环付费席，2026-08-31 定稿）
```
POST /explore/boost/order
```
**功能**：为指定坐标购买置顶席位。两档：周卡 7元/7天，月卡 60元/30天（王总定稿：同坐标持卡期间不可叠加再买）。走半自动人工确认（复用会员订单确认链路）。

**约束**：已有活跃席位拒绝重复购买（409）；同一用户全局一次只能有一笔待确认订单（与会员下单共用幂等）。

| 参数 | 类型 | 说明 |
|------|------|------|
| coordKey | String | 坐标标题（Coord.title） |
| tier | String | week（缺省）/ month |

**返回**：`{ orderId, orderNo, amount, status, payeeQrCodeUrl, remark, coordKey }`

### 5.4 查询置顶订单（轮询）
```
GET /explore/boost/order/{orderId}
```
**功能**：查询置顶订单状态（复用会员订单查询，含归属校验）。前端下单后每 3 秒轮询，status 为 paid 即生效。

**返回**：订单对象（含 coordKey、expireAt；paid 时 expireAt = 置顶到期时间）。

**置顶规则**（排序逻辑在探索弹窗前端）：五席一循环：月卡席 → 周卡席 → 高赞席 → 最新席 → 高赞席（高赞每轮占两席）；同档多人按 start（确认时刻）倒序即后买靠前；空席跳过。探索坐标卡 boostAuthors 字段为对象 `{ month: [], week: [] }`（各档活跃席位作者昵称，后买靠前序）。
...
## 15. 雷达（实时位置共享，2026-09-20 一期）

> 一期为前台版：摄影师在**工作模式 + 未隐身 + 应用前台**时周期性上报位置；
> 查看端以自己为中心显示半径内在线摄影师（紫色脉冲点，可点进主页）。
> 二期预留：原生前台服务插件（UTS，仿 uts-screencontrol）实现退后台继续上报。

**数据语义**：每人只存一条「最后位置 + 上报时间」（radar_locations，upsert 覆盖）；
最后上报距今超过 **60 秒**视为离线（查询侧用 updatedAt 过滤，数据不删）。
**可见性过滤**：仅 `mode=work` 且 `radarHidden=false` 且心跳 60 秒内的用户可见；
隐身 = 查询过滤 + 上报拒绝（双保险）。
...
POST /radar/report
```
**功能**：工作模式摄影师上报当前位置（每人一条，覆盖写）。
服务端防御：同一用户限流 ≤1 次/3 秒；与上次位置换算速度 >120km/h 视为异常丢弃（均静默成功 `accepted:false`，不返回错误，避免客户端重试风暴）。

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| lng | Float | 是 | 经度（-180~180） |
| lat | Float | 是 | 纬度（-90~90） |
| accuracy | Float | 否 | 定位精度（米，默认 0） |

**返回**：
```json
{ "data": { "accepted": true } }
```
`accepted:false` 的 reason：`rate_limited`（限流）/ `jump`（跳变异常）/ `hidden`（隐身中）。

### 15.2 获取附近在线摄影师
```
GET /radar/nearby?lng=..&lat=..&radius=5000
```
**功能**：以指定点为中心，返回半径内在线摄影师（按距离升序）。
过滤：排除自己、仅工作模式、未隐身、最后上报 <60 秒、半径内（bbox 预过滤 + 圆形精确过滤）。
lng/lat 可省略（不限范围）；radius 约定同探索模式（≤180 视为度，>180 视为米，默认 5000）。
...
```
**功能**：切换雷达隐身。开启 = 不出现在他人雷达中（nearby 过滤 + report 拒绝，双保险）。

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| hidden | Boolean | 是 | true 隐身 / false 恢复可见 |

**返回**：
```json
{ "data": { "radarHidden": true }, "message": "已隐身，雷达不再显示你的位置" }
```

**其他联动**：`GET /users/me` 返回新增 `radarHidden` 字段（前端初始化隐身开关状态）。
