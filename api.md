...
### 5.3 购买坐标置顶（三赛道付费席，2026-08 定稿）
```
POST /explore/boost/order
```
**功能**：为指定坐标购买置顶席位（6元 / 7天）。走半自动人工确认（复用会员订单确认链路）。

**约束**：该坐标名下须有本人获赞不低于1的未删除照片（质量门槛）；已有活跃席位拒绝重复购买（409）；同一用户全局一次只能有一笔待确认订单（与会员下单共用幂等）。
...
**置顶规则**（排序逻辑在探索弹窗前端）：付费席按 start（确认时刻）倒序即后买靠前（排位赛 C 大于 B 大于 A）；席位未过期者永远高于从未购买者。探索坐标卡新增 boostAuthors 字段（数组，按后买靠前序）注入活跃席位作者昵称。
