/*!
 * bridge-transport.js r70 -- App 内（uni-app web-view）UTS 原生 USB 桥接传输层
 *
 * 与 browser-usb-transport.js（WebUSB 版）接口同构：本文件在 App web-view 环境中
 * 检测到 uni.postMessage 可用时覆盖全局 UsbTether（非 App 环境不注册，
 * WebUSB 版继续生效，PC/Chrome 行为零改动）。
 *
 * r70 修复（真机实锤）：uni 桥注入晚于本脚本执行（phone-link 13ea161 同款问题）--
 * r69 在脚本加载时一次性检测，App 内 window.uni 尚未注入 -> 桥永不注册 ->
 * USB 走 plus 版（Native.js 死路）报 "transport.clearPipe is not a function"。
 * 改为三通道延迟安装：① 立即尝试 ② 500ms 轮询 60 次（30s）③ App 侧
 * connect.vue evalJS __usbAppBridgeReady() 主动通知（最可靠）。
 * 安装成功 dispatch 'usbtether-installed'，页面据此自动重扫（若 USB 弹层开着）。
 *
...
(function () {
  ...
  function install() {
    ...
    function BridgeTransport() {
      this.bulkInCap = 1048576; // 原生 bulkTransfer 单次可读 1MB（返回实际长度，短包语义）
    }
    ...
    UsbTetherUts.prototype.utsMode = true;     // connect-prototype.html：scan 分支与文案适配
    ...
    window.UsbTether = new UsbTetherUts(); // 覆盖：App 内原生桥优先
    installed = true;
    try {
      window.dispatchEvent(new CustomEvent('usbtether-installed'));
      try { console.log('[usb-bridge] UTS 桥已安装（utsMode）'); } catch (e) {}
    } catch (e) {}
    return true;
  }

  // uni 桥是否可用（App web-view 内注入时机不稳定，须动态检测）
  function uniReady() {
    return !!(window.uni && typeof window.uni.postMessage === 'function');
  }

  // 三通道安装：① 立即 ② 轮询（uni 桥晚注入，30s 内持续检测）③ App 侧主动通知
  function tryInstall() {
    if (installed) return true;
    if (!uniReady()) return false;
    return install();
  }
  ...
  if (!tryInstall()) {
    ...
    var timer = setInterval(function () {
      n++;
      if (tryInstall() || n >= 60) clearInterval(timer); // 500ms × 60 = 30s
    }, 500);
  }

  // App 侧（connect.vue forceAppMode 节拍）主动通知：最可靠通道
  window.__usbAppBridgeReady = function () { tryInstall(); };
})();
