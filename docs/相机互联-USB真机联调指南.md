...
> **当前状态（2026-08-16）**：排查六层根因已全清；此后按 gphoto2/RemoteYourCam/JPhoto 成功案例对协议栈做了**全面重写**（见「五、技术说明」），**待王总真机复验**。授权弹窗已实证通过；复验目标：检测 → 授权 → OPEN_SESSION 成功 → 进工作台 → 快门 → 照片落待归类管线。
...
| **点设备后长时间停在「连接中」** | **（22ce574 待复验，重点反馈项）** 点「USB诊断」按钮，把弹窗 JSON 截图发给开发者——重点看：`usbVer`（应为 r11+）、`ifaceInfo`（应为 class=6 + 0x81 IN / 0x02 OUT / 0x83 中断 IN）、连接阶段 n 值（-1 为 STALL，预期已由 clear halt 兜底重试） |
...
- 协议栈单测：`node script/test-ptp.js`（mock 相机全链路 31/31，流式模拟真实 USB 切分）
...
| 快门 0x910F | 按下=0→120ms→释放=1 两段式 | **无参数单发**，响应 Param1 结果码（0 成功/1 对焦失败/3 反光板/7 卡满/8 只读） |
